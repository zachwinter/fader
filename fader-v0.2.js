	...
	$.fn.fader = function(options){
		...
		*/
		...
		var prevNextBind = function(){
			if (settings.prevButton != false) {
				settings.prevButton.on('click', function(e){
					...
					if (changing != true) updateActive(false);
					e.preventDefault();
				});
			}
			if (settings.nextButton != false) {
				settings.nextButton.on('click', function(e){
					...
					if (changing != true) updateActive(true);
					e.preventDefault();
				});
			}
		};
		...
		var updateActive = function(next){
			if (settings.bullets != false) settings.bullets.find('a').removeClass('active');
			if (next == true) {
				if (active == slides.length) {
					if (settings.bullets != false) settings.bullets.find('[data-target="1"]').addClass('active');
					swapSlide(1);
				} else {
					if (settings.bullets != false) settings.bullets.find('[data-target="'+(active+1)+'"]').addClass('active');
					swapSlide(active+1);
				}
			} else {
				if (active == 1) {
					if (settings.bullets != false) settings.bullets.find('[data-target="'+slides.length+'"]').addClass('active');
		...
		var swapSlide = function(slide){
			...
			if (settings.fixedAspect == false) container.css('height', container.height());
			...
			self.finishUp = function(){
				...
				if (settings.fixedAspect == false) container.css('height', 'auto');
				active   = slide;
				changing = false;
				...
				if (settings.autoSwap == true && autoDisabled == false) auto();
			};

			// Call rendering method based on CSS Transition support.
			transitionSupport == true ? self.cssAnimation() : self.jQueryAnimation();
		...
		var auto = function(){
			...
			timer = setTimeout(function(){
				if (changing == false) updateActive(true);
			}, settings.swapDelay);
		};
		...
		var resizeDisable = function(){
			...
			self.trigger = function(){
				changing = false;
				if (settings.autoSwap == true) auto();
			}
		...
		*/
		...
		if (container.length) {
			...
			if (containerPosition != "absolute" || containerPosition != "relative" || containerPosition != "fixed") container.css('position', 'relative');
			...
			// Bind keyboard.
			if (settings.keyboard == true) keyboardBind();	
	
			// Bind prev/next buttons.
			if (settings.prevButton != false || settings.nextButton != false) prevNextBind();
	
			// Start automatic slide swapping.
			if (settings.autoSwap == true) auto();
	
			// Enable the prevention of slides changing during window resize.
			if (settings.disableOnResize == true) resizeDisable();
